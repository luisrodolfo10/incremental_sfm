...
from sfm.utils import build_intrinsic_matrix, save_reconstruction_points, save_cameras_np
from sfm.gui import ReconstructionViewer
import pandas as pd
...
LOOSE_REPROJ_THRESH = 15.0                  # Loose pixel error to filter obvious reprojection errors  
...
F_TOL = 1e-5
G_TOL = 1e-5
N_POINTS = 70                              # Limit of N points for each triangulation (50 for big ones) 
...
def incremental_sfm(N_IMGS, feature_file, K, vis=None):
    ...
    viewer = ReconstructionViewer(pcd_output_fine, cameras_output_fine + ".npz", img_size=img_size, imgs_paths=imgs_paths)
    viewer.run()
...
if __name__ == "__main__":
    ...
    ######################################################################
    ############################ Starting SfM ############################
    incremental_sfm(N_IMGS, feature_file, K)
